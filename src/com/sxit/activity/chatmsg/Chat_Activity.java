...
//			ui.setUsername(getLoginUser().getRealname());
			...
			String[] property_nm = { "fromUser", "toUser", "blogTitle", "blogAbstract", "content", "devType",
					"infoType", "infoDirection" };
			Object[] property_va = { analystInfo.getAdv_user_id(), getLoginUser().getUserid(), content, content,
					content, 1, 1, 1 };
	...
	class SendAT extends AsyncTask<Object, Object, Object> {
		...
		protected Object doInBackground(Object... params) {
			...
			Object res_obj = SoapWebService.data(SOAP_UTILS.METHOD.INTERACTIONSUBMIT, (String[]) params[0],
					(Object[]) params[1]);
		...
		protected void onPostExecute(Object result) {
			...
			String userhearttempStr = "" + (Integer.parseInt(userheart.getText().toString().trim())
					- Integer.parseInt(analystInfo.getHeartcount().toString().trim()));
...
//		ui.setUsername(analystInfo.getRealname());
	...
	protected void onNewIntent(Intent intent) {
		...
		if (analystInfo != null) {
			...
			// }else{
			// analystInfo =
			// dbh.queryAdviserInfoById(analystInfo.getAdv_user_id());
			// }
			if (analystInfo.getHeadpic() == null || analystInfo.getHeadpic().equals("")) {
				if (analystInfo.getAdv_user_id().equals("1")) {
					analystInfo = dbh.queryAdviserGz();
		...
		userheart = (TextView) findViewById(R.id.userheart);
		heartcount.setText(analystInfo.getHeartcount());
		// Toast.makeText(Chat_Activity.this, "" + analystInfo.getHeartcount(),
		// Toast.LENGTH_SHORT).show();
