...
public class PublishMsg_Adapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		Instance.imageLoader.displayImage(SOAP_UTILS.HEADER_URL + ((BaseActivity) context).getLoginUser().getHeadpic(),
				holder.img_head_left, Instance.user_options);
		if (publishMessages.get(position).getPhoto().equals(SOAP_UTILS.PIC_YANBAO + "no.jpg")) {// no
																								// image
			holder.img_content_url_left.setVisibility(View.GONE);
		} else {
			Instance.imageLoader.displayImage(publishMessages.get(position).getPhoto(), holder.img_content_url_left,
