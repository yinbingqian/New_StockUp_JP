 ...
 */
public class StandPoint_Activity extends BaseActivity {
	Context context;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_standpoint);
	...
	private void setListeners() {
		...
		listView_newslist.setOnLastItemVisibleListener(new OnLastItemVisibleListener() {
			...
			public void onLastItemVisible() {
				String[] property_va = new String[] { "10", ++pageIndex + "" };
				soapService.getListByGuest(property_va, true);
	...
	public void onEvent(SoapRes obj) {
		super.onEvent(obj);
		if (obj.getCode().equals(SOAP_UTILS.METHOD.GETLISTBYGUEST)) {
			...
			if(obj.isPage()){
				...
				if(list.size()!=0){					
					...
					dbh.clearStandPointData();
					dbh.insStandPointInfo(list);
					pageIndex = 1;
				}
				getDBData();
			}
