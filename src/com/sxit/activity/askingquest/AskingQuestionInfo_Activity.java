...
public class AskingQuestionInfo_Activity extends BaseActivity {
	...
	private void setListeners() {
		...
		gridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if (Integer.parseInt(moneys[position]) <= Integer.parseInt(getLoginUser().getMark())
						|| Integer.parseInt(moneys[position]) == 0) {
					for (int i = 0; i < parent.getCount(); i++) {
						View v = parent.getChildAt(i);
						if (position == i) {// 当前选中的Item改变背景颜色
							view.setBackgroundResource(R.drawable.grid_select);
							ViewHolder_Info holder = (ViewHolder_Info) view.getTag();
							holder.tv_money.setTextColor(
									AskingQuestionInfo_Activity.this.getResources().getColor(R.color.white));
							...
							holder.tv_money.setTextColor(AskingQuestionInfo_Activity.this.getResources()
									.getColor(R.color.register_text_color));
						}
					}
					money = "0";
	...
	public void onClick(View v) {
		...
		case R.id.tv_cancel:
			...
			break;
		case R.id.bt_pay:
			// intent.setClass(AskingQuestionInfo_Activity.this,
