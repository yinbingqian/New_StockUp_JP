...
public class Information_Adapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		Instance.imageLoader.displayImage(SOAP_UTILS.HEADER_URL + list.get(position).getImgUrl(), viewHolder.img_icon,
		...
		if (list.get(position).getAdviser() != null) {
			if (list.get(position).getAdviser().getAdv_user_id().toString().trim().equals("1")) {
				viewHolder.tv_title.setText("股涨运营团队");
			}
			System.out.println("### viewHolder.tv_title = " + viewHolder.tv_title.getText() + " and id = "
					+ list.get(position).getAdviser().getAdv_user_id());
		}
