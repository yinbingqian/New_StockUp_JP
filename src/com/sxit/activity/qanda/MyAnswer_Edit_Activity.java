 ...
 */
public class MyAnswer_Edit_Activity extends BaseActivity {
	Context context;
...
//		Toast.makeText(context, img1_name+"|"+img2_name + "|" + tag_name, Toast.LENGTH_SHORT).show();
		if(edit_content.getText().toString().trim().equals("")){
			Toast.makeText(context, "解答内容不能为空", Toast.LENGTH_SHORT).show();
			return;
		}
//		String[] property_va = new String[] { answer.getwebid(), user.getid(), edit_content.getText().toString(), img1_name, img2_name };
//		soapService.answerSubmit(property_va);
		String[] property_nm = { "quesid","userid", "content", "img1", "img2" };
		Object[] property_va = { answer.getwebid(), user.getid(), edit_content.getText().toString(), img1_name, img2_name };
		new answerSubmitPostPostTask().execute(property_nm, property_va);
	}
	
	class answerSubmitPostPostTask extends AsyncTask<Object, Object, Object> {
        @Override
        ...
        protected Object doInBackground(Object... params) {
            System.out.println(">>>>>");
            Object res_obj = (Object) HttpPostService.data(SOAP_UTILS.METHOD.ANSWERSUBMITPOST, (String[]) params[0],(Object[]) params[1]);
            return res_obj;
        }
        ...
        protected void onPostExecute(Object result) {
            ...
            try {
				...
				JSONObject json_obj = new JSONObject(json_res.get("AnswerSubmitPostResult").toString());
	...
	private void initView() {
		...
		name = (TextView) findViewById(R.id.answer_name);
		headpic = (ImageView) findViewById(R.id.answer_headpic);
		title = (TextView) findViewById(R.id.answer_title);
		content = (TextView) findViewById(R.id.answer_content);
		time = (TextView) findViewById(R.id.answer_time);
		gridview = (NoScrollGridView) findViewById(R.id.answer_gridview);
		
		bt_send = (Button) findViewById(R.id.bt_send);
		tv_tagname = (TextView) findViewById(R.id.tag_name);
		
		name.setText(answer.getrealname());
		Instance.imageLoader.displayImage(
				SOAP_UTILS.HTTP_HEAD_PATH + answer.getheadpic(), headpic, Instance.user_s_options);
		title.setText(answer.gettitle());
		// viewHolder.intro.setTextColor(context.getResources().getColor(R.color.text_color));
		content.setText("问题描述：" + answer.getcontent());
		time.setText(answer.getcrtime() + "    [" + answer.getcolumn() + "]");

		String img1 = answer.getimg1();
		String imgthumbnail1 = answer.getimgthumbnail1();
		String img2 = answer.getimg2();
		String imgthumbnail2 = answer.getimgthumbnail2();
		...
		if (imgthumbnailUrls == null || imgthumbnailUrls.size() == 0) { // 没有图片资源就隐藏GridView
			...
			gridview.setAdapter(new NoScrollGridAdapter(context, imgthumbnailUrls));
		}
	...
	public void onEvent(SoapRes obj) {
		super.onEvent(obj);
		if (obj.getCode().equals(SOAP_UTILS.METHOD.ANSWERSUBMIT)) {
        ...
        protected void onPostExecute(Object result) {
            ...
            try {
				...
				if(json_obj.get("Result").toString().equals("success")){
					...
					Toast.makeText(MyAnswer_Edit_Activity.this, "图像上传成功", Toast.LENGTH_SHORT).show();
				}else if(json_obj.get("Result").toString().equals("error")){
					Toast.makeText(MyAnswer_Edit_Activity.this, json_obj.get("Message").toString(), Toast.LENGTH_SHORT).show();
				}
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
